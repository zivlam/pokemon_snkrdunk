...
def check_dns(hostname):
    """Diyagnostiko ti kasasaad ti resolusion ti DNS"""
    try:
...
def format_to_hk_time(utc_date_str):
    """Ibaliw ti kuerdas ti oras nga UTC iti oras ti Hong Kong (UTC+8)"""
...
def get_snkrdunk_trading_history(product_id, page=1, per_page=12, filter_psa10=True):
    """
    Alaen ti pakasaritaan ti panagtagilako dagiti Tarheta ti SNKRDUNK ket isagat ti PSA10, nga agpataud iti HKD a kakkuyogna
    """
    hostname = "snkrdunk.com"
    HKD_RATE = 0.051 # I-set ti patubong ti Yen iti HKD
    
    ip = check_dns(hostname)
    if not ip:
        print(f"【Ballaag ti Aglawlaw ti Network】Saan a ma-resolve {hostname}。")
        return
    ...
    try:
        print(f"Kiddawen ti datos (ID: {product_id}, Panid {page})...")
        ...
        if response.status_code == 200:
            ...
            if not histories:
                print("【Ballaag】Awan ti listaan ti datos a masarakan.")
                return

            # Manginayon ti format ti HKD (Inayon ti pormat ti HKD)
            print(f"{'Petsa (HKT)':<20} | {'Kasasaad':<10} | {'HKD':<12} | {'JPY'}")
            print("-" * 65)
            ...
            if found_count == 0:
                print("【Pammagbaga】Awan ti rekord ti PSA 10 iti daytoy a panid.")
        else:
            print(f"【Biddut】Kodigo ti kasasaad: {response.status_code}")
            
    except Exception as e:
        print(f"【Adda riribuk】: {e}")

if __name__ == "__main__":
    # Suboken amin a baro a tarheta a nairaman
    cards_to_track = [
